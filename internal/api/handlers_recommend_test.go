...
// mockRecommendEngine implements the minimal interface needed for testing.
type mockRecommendEngine struct {
	recommendResponse *recommend.Response
	recommendErr      error
	status            recommend.TrainingStatus
	metrics           recommend.Metrics
...
func (m *mockRecommendEngine) Recommend(_ context.Context, _ recommend.Request) (*recommend.Response, error) {
	if m.recommendErr != nil {
		return nil, m.recommendErr
	}
	return m.recommendResponse, nil
}

func (m *mockRecommendEngine) GetStatus() recommend.TrainingStatus {
	return m.status
}

func (m *mockRecommendEngine) GetMetrics() recommend.Metrics {
	return m.metrics
}

func (m *mockRecommendEngine) GetConfig() *recommend.Config {
	return m.config
}

func (m *mockRecommendEngine) UpdateConfig(_ *recommend.Config) error {
	return m.configUpdateErr
}

func (m *mockRecommendEngine) Train(_ context.Context) error {
	return m.trainErr
}

func (m *mockRecommendEngine) SetDataProvider(_ recommend.DataProvider) {}

// testRecommendHandler creates a test handler with mock engine.
type testRecommendHandler struct {
	engine *mockRecommendEngine
	db     *mockRecommendDB
}

// mockRecommendDB implements minimal database interface for recommend handler tests.
type mockRecommendDB struct {
	continueWatchingItems []recommend.ScoredItem
...
func (m *mockRecommendDB) GetContinueWatchingItems(_ context.Context, _ int, _ int) ([]recommend.ScoredItem, error) {
	return m.continueWatchingItems, m.continueWatchingErr
}

func (m *mockRecommendDB) GetMediaItemByID(_ context.Context, _ int) (*mockMediaItem, error) {
	return m.mediaItem, m.mediaItemErr
}

func newTestRecommendHandler() *testRecommendHandler {
	return &testRecommendHandler{
		engine: &mockRecommendEngine{
			recommendResponse: &recommend.Response{
				Items: []recommend.ScoredItem{
					{Item: recommend.Item{ID: 1, Title: "Test Movie 1"}, Score: 0.95},
					{Item: recommend.Item{ID: 2, Title: "Test Movie 2"}, Score: 0.85},
				},
				Metadata: recommend.ResponseMetadata{
					LatencyMS:    15,
					ModelVersion: 1,
				},
			},
			status: recommend.TrainingStatus{
				IsTraining:    false,
				ModelVersion:  1,
				LastTrainedAt: time.Now().Add(-time.Hour),
			},
			metrics: recommend.Metrics{
				RequestCount:     100,
				AverageLatencyMS: 10,
		...
		db: &mockRecommendDB{
			continueWatchingItems: []recommend.ScoredItem{
				{Item: recommend.Item{ID: 10, Title: "In Progress Movie"}, Score: 0.5},
			},
		},
	}
}
