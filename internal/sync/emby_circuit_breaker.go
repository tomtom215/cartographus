...
// Ensure EmbyCircuitBreakerClient implements EmbyClientInterface
var _ EmbyClientInterface = (*EmbyCircuitBreakerClient)(nil)

// EmbyCircuitBreakerClient wraps EmbyClient with circuit breaker pattern
// Prevents cascading failures when Emby API is unavailable or slow
//
...
type EmbyCircuitBreakerClient struct {
	client *EmbyClient
	cb     *gobreaker.CircuitBreaker[interface{}]
	name   string
}

// EmbyCircuitBreakerConfig holds configuration for the Emby circuit breaker
type EmbyCircuitBreakerConfig struct {
	BaseURL string
	APIKey  string
	UserID  string
}

// NewEmbyCircuitBreakerClient creates a new Emby client with circuit breaker
...
func NewEmbyCircuitBreakerClient(cfg EmbyCircuitBreakerConfig) *EmbyCircuitBreakerClient {
	client := NewEmbyClient(cfg.BaseURL, cfg.APIKey, cfg.UserID)
	cbName := "emby-api"
	...
	cb := gobreaker.NewCircuitBreaker[interface{}](gobreaker.Settings{
		...
		ReadyToTrip: func(counts gobreaker.Counts) bool {
			...
			if shouldTrip {
				logging.Warn().Uint32("failures", counts.TotalFailures).Float64("failure_rate", failureRatio*100).Msg("[CIRCUIT BREAKER] Opening Emby circuit")
			}
		...
		OnStateChange: func(name string, from, to gobreaker.State) {
			...
			logging.Info().Str("from", fromStr).Str("to", toStr).Msg("[CIRCUIT BREAKER] Emby state transition")
	...
	return &EmbyCircuitBreakerClient{
		client: client,
		cb:     cb,
		name:   cbName,
	}
...
// execute wraps an Emby API call with circuit breaker protection
func (cbc *EmbyCircuitBreakerClient) execute(fn func() (interface{}, error)) (interface{}, error) {
	...
	if err != nil {
		if errors.Is(err, gobreaker.ErrOpenState) || errors.Is(err, gobreaker.ErrTooManyRequests) {
			...
			logging.Warn().Err(err).Msg("[CIRCUIT BREAKER] Emby request rejected")
		} else {
...
// Ping tests connectivity to the Emby server with circuit breaker protection
func (cbc *EmbyCircuitBreakerClient) Ping(ctx context.Context) error {
...
func (cbc *EmbyCircuitBreakerClient) GetSessions(ctx context.Context) ([]models.EmbySession, error) {
	...
	sessions, ok := result.([]models.EmbySession)
	if !ok {
...
func (cbc *EmbyCircuitBreakerClient) GetActiveSessions(ctx context.Context) ([]models.EmbySession, error) {
	result, err := cbc.execute(func() (interface{}, error) {
...
// GetSystemInfo retrieves Emby server system information with circuit breaker protection
func (cbc *EmbyCircuitBreakerClient) GetSystemInfo(ctx context.Context) (*EmbySystemInfo, error) {
	result, err := cbc.execute(func() (interface{}, error) {
...
func (cbc *EmbyCircuitBreakerClient) GetUsers(ctx context.Context) ([]EmbyUser, error) {
	result, err := cbc.execute(func() (interface{}, error) {
...
func (cbc *EmbyCircuitBreakerClient) StopSession(ctx context.Context, sessionID string) error {
	_, err := cbc.execute(func() (interface{}, error) {
...
func (cbc *EmbyCircuitBreakerClient) GetWebSocketURL() (string, error) {
	return cbc.client.GetWebSocketURL()
}

// State returns the current circuit breaker state
func (cbc *EmbyCircuitBreakerClient) State() gobreaker.State {
	return cbc.cb.State()
}

// Counts returns the current circuit breaker counts
func (cbc *EmbyCircuitBreakerClient) Counts() gobreaker.Counts {
	return cbc.cb.Counts()
}

// Name returns the circuit breaker name
func (cbc *EmbyCircuitBreakerClient) Name() string {
	return cbc.name
}
